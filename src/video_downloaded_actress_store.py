"""Persist video-downloaded actress marks on JavDB profile pages."""
...
def init_video_downloaded_actress_db() -> None:
    init_sticker_db()
    with get_connection() as conn:
        conn.executescript(
            """
            CREATE TABLE IF NOT EXISTS video_downloaded_actresses (
            ...
            CREATE INDEX IF NOT EXISTS idx_video_downloaded_actress_name
                ON video_downloaded_actresses(name);
            """
        )
...
def record_video_downloaded_actress(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    with get_connection() as conn:
        conn.execute(
            """
            INSERT INTO video_downloaded_actresses (
...
def get_video_downloaded_actresses_list() -> list[dict[str, Any]]:
    init_video_downloaded_actress_db()
    with get_connection() as conn:
        rows = conn.execute(
            """
            SELECT javdb_id, name, folder_name, profile_url, recorded_at
            FROM video_downloaded_actresses
