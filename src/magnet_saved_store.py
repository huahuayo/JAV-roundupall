"""Write magnet-saved sync summary logs."""
...
FOLDER_LOG_PREFIX = "磁链已保存同步"

ROOT_LOG_PREFIX = "磁链已保存同步总览"
...
def _build_media_summary(sorted_results: list[dict]) -> list[str]:
    ...
    four_k_no_sub = _code_list(

        sorted_results,

        lambda item: item.get("is_4k") and not item.get("four_k_has_subtitle"),

    )

    four_k_sub_file = _code_list(sorted_results, lambda item: item.get("four_k_has_subtitle_file"))

    four_k_sub_ch = _code_list(sorted_results, lambda item: item.get("four_k_has_subtitle_via_ch"))

    four_k_sub_ready = _code_list(sorted_results, lambda item: item.get("four_k_has_subtitle"))

    subtitle_extract = _code_list(sorted_results, lambda item: item.get("subtitle_extract_for_4k"))
    ...
    return [
        ...
        f"# 4K 无字幕 ({len(four_k_no_sub)}): {_join_codes(four_k_no_sub)}",

        f"# 4K 有字幕 · 本地 srt/ass ({len(four_k_sub_file)}): {_join_codes(four_k_sub_file)}",

        f"# 4K 有字幕 · 同番号 ch/c 磁链 ({len(four_k_sub_ch)}): {_join_codes(four_k_sub_ch)}",

        f"# 4K 可烧录字幕 ({len(four_k_sub_ready)}): {_join_codes(four_k_sub_ready)}",

        f"# 为 4K 提取字幕而保存的 ch/c 磁链 ({len(subtitle_extract)}): {_join_codes(subtitle_extract)}",
...
def write_magnet_saved_folder_log(folder_path: str, *, session_id: str, actress_name: str, code_results: list[dict]) -> str:
    ...
    for item in sorted_results:
        ...
        subtitle = _subtitle_label(item)

        four_k = _four_k_label(item)

        if item.get("ok"):

            status = "成功"

            detail = "已标记「已下载」、详情页磁链/字幕标签，并写入桌面数据库"
        ...
        lines.append(f"{code} | {status} | {subtitle} | {four_k} | {detail}")
    ...
    try:
        ...
        logger.info("Wrote magnet saved folder log: %s", log_path)
...
def write_magnet_saved_root_logs(
    ...
    for root in log_roots:
        ...
        for item in folder_results:
            ...
            if storage_type == "115":
                ...
                storage_note = "已保存到115"

            elif storage_type == "local_magnet":

                storage_note = "已保存磁链到本地"



            if item.get("ok") or int(item.get("success_codes") or 0) > 0:

                detail = "同步完成" if fail == 0 else f"部分完成（{fail} 个未同步）"

                if storage_note:

                    detail += f"；女优页标记「{storage_note}」"
        ...
        try:
            ...
            logger.info("Wrote magnet saved root log: %s", log_path)
